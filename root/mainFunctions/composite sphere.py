...
while H[0]<0 or H[1]<0:
	H[0]=float(input(('Enter heat transfer coefcient H_a in W/m^2K : ')))
	H[1]=float(input(('Enter heat transfer coefcient H_b in W/m^2K : ')))
print()	

# Ambients temperatures input
print('AMBIENT TEMPERATURES : ')
while  (-1 in T_amb):
	T_amb[0] = float(input('Enter ambient temperature T_a in *C : '))
	T_amb[1] = float(input('Enter ambient temperature T_b in *C : '))
	if -1 in T_amb:
...
if (-1 in T) and n_layers>1:
	...
	print("Insufficient data (Temperatures) !!!")
